...
(function() {
	...
	$.fn.treetable = function(method, id) {
		if (methods[method]) {
...
jQuery.TreeTable = function(tree_id, heads, tNodes) {
	if ($("#" + tree_id).length > 0) {
		...
		for (var i = 0, l = newNodes.length; i < l; i++) {
			var node = newNodes[i];
			body_html += "<tr onclick=\"ck('" + node.id + "','" + node.name + "','" + node.pId + "')\" data-tt-id='" + node.id + "' "
					+ (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") + "><td class = 'td_left' title='" + node.name + "'><span class='"
					+ (node.isParent ? "parent" : "children") + "'>"
					+ "<a class='hrefclass' style='text-decoration: none;' href=\"javascript:void(0)\">" + node.name + "</a>" + "</span></td>";
			for (var j = 0; j < column_count - 1; j++) {
				body_html += "<td title='" + (node.td[j] ? node.td[j].replace(/\<[^<>]+\>/g,'') : "") + "'>" + (node.td[j] ? node.td[j] : "") + "</td>";
			}
			body_html += "</tr>";
		}
		body_html += "</tbody>";
